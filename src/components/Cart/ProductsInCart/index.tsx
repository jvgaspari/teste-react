    ...
    return (
        <Box flex={1} bgcolor='#FFFFFF' justifyContent={'center'} alignItems='center' p='24px' borderRadius={'4px'}>
            <HeaderCart/>
            {
                list.map((product) => (
                    <>
                        <Grid sx={{ display: {xs: 'none', md: 'flex'}}} container spacing={2} key={product.id} alignItems='center' mb='8px'>
                            <Grid item xs={6}>
                                <Box display={'flex'} >
                        ...
                        <Grid sx={{display: {sx: 'flex', md: 'none'}}} container spacing={2} key={product.id} alignItems='center' mb='16px'>
                            <Grid item xs={3}>
                                <Box display={'flex'} >
                                    <Image src={product.image} width={64} height={82} alt={product.title} />
                                </Box>
                            </Grid>
                            <Grid item xs={9}>
                                <Box ml='16px' height={'20px'} display={'flex'} justifyContent={'space-between'} alignItems='center' >
                                    <Typography color={'#2F2E41'} fontWeight={700} fontSize={14} lineHeight={'19px'}>
                                        {product.title}
                                    </Typography>
                                    <Typography color={'#2F2E41'} fontWeight={700} fontSize={14} lineHeight={'19px'}>
                                ...
                                <Box ml='16px' mt={'23px'} display={'flex'} justifyContent={'space-between'} alignItems='center'>
                                    <Box  display={'flex'} justifyContent={'space-between'} alignItems='center'>
                                        <IconButton 
                                            style={{zIndex: 1}} 
                                            onClick={() => decreaseQuantity(product.id)}
                                        ...
                                        <Box border='1px solid #D9D9D9' borderRadius= '4px' flex={1} minWidth='24px' display={'flex'} justifyContent={'center'} alignItems='center'>
                                            <Typography>{product.quantity}</Typography>
                                        </Box>
                                        <IconButton style={{zIndex: 1}} onClick={() => incrementQuantity(product.id)}>
                                            <Increase/>
                                    ...
                                    <Box display={'flex'} flexDirection='column' justifyContent={'flex-end'}>
                                        <Typography color={'#999999'} fontWeight={700} fontSize={12} lineHeight={'16.34px'}>
                                            SUBTOTAL
                                        </Typography>
                                        <Typography color={'#2F2E41'} fontWeight={700} fontSize={16} lineHeight={'21.79px'} alignSelf={'flex-end'}>{Mask('MoneyMask', String((product.price * product.quantity).toFixed(2)))}</Typography>
                                    </Box>
                                </Box>
                            </Grid>
                        </Grid>
